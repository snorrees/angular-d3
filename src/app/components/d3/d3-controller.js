...
angular.module('angularD3')
  .controller('D3Controller', function (d3Service, $scope) {
    ...
    model.labels = [
      ...
      {label: 'Cross-section', property: 'cross'}
    ];
    ...
    function updateSlider() {
      model.slider = {
        max:  _.chain(data).map(model.cross).uniq().value().length,
    ...
    function updateAll() {
      updateSlider();
      model.data = subData(model.cross, model.slider.ticks[model.slider.value]);
    }
